...
from freedpp.env.reward import Reward, MolLogP, PatternFilter, OutOfRange, identity, Brenk, qed_mol, SA, CATS_Euclid, CATS_Cosine, MACCS_Tanimoto, map4

from freedpp.env.docking import DockingVina
from freedpp.env.environment_scaffold import Environment
...
def init_rewards(args):
    ...
    costs = {
        ...
        'SA': Reward(SA, OutOfRange(lower=0, upper=3, hard=hard), preprocess=preprocess),
        'Brenk': Reward(Brenk, identity, preprocess=preprocess),
        'qed_mol': Reward(qed_mol, OutOfRange(lower=0.75, upper=1, hard=hard), preprocess=preprocess),
        ...
        'CATS_Euclid': Reward(CATS_Euclid, OutOfRange(lower=0.0, upper=10, hard=hard), preprocess=preprocess),
        'CATS_Cosine': Reward(CATS_Cosine, OutOfRange(lower=0.8, upper=1.0, hard=hard), preprocess=preprocess),
        'MACCS_Tanimoto': Reward(MACCS_Tanimoto, OutOfRange(lower=0.0, upper=0.5, hard=hard), preprocess=preprocess),
        'map4': Reward(map4, OutOfRange(lower=0.0, upper=0.5, hard=hard), preprocess=preprocess),
    }
    rewards = dict()
